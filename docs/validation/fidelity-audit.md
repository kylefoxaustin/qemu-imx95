...
| **Mali GPU** | FAULTS (honest) | `gpu@4d900000` is a reads-as-0 stub; the kbase DDK (`r54p1`) loads but rejects probe with -EINVAL ("Unknown GPU Product ID 0") — GL code fails fast, not silently. Registration-tier bring-up (report a valid Mali-G310 GPU_ID so Linux *sees* the GPU, compute honest-flagged) is scheduled: the GPU_ID gate is cracked locally, remaining work is the CSF reset/power handshake (the userspace is Arm's proprietary libmali, so GL can't run regardless). |
