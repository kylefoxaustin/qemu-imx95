...
| **Mali GPU** | IDENTIFIED → FAULTS (honest) | `gpu@4d900000` = `imx95_mali.c`, an identify-tier model: it reports the real Mali-G310 GPU_ID (TVAX, arch 10.12.7 = `0xAC740001`) so the kbase DDK (`r54p1`) **identifies** the GPU (`GPU identified as 0x4 arch 10.12.7`) and reads its property registers, then fails the un-modelled CSF bring-up **cleanly** with -EIO (`Miscellaneous device initialization failed`) — no hang, no fake `/dev/mali0`, boot reaches userspace. Linux sees + names the GPU and the driver fails honestly. Full bind (CSF reset/power/MCU-firmware handshake) is out of scope: the userspace is Arm's proprietary libmali, so GL can't run regardless. GPU_ID guarded by `tests/qtest/imx95-mali-test.c`. (NB: a wrong arch_minor builds an incomplete kbase regmap → NULL-deref oops in gpuprops; 10.**12**.7 is the real TVAX revision, read back from silicon.) |
...
| **Thermal sensors (SCMI)** | honest (verified 2026-07-11, NOT silent-wrong) | Five SCMI thermal zones served by the SM. **TMPSNS** (`ana`, `a55-thermal`; `fsl-imx95.c` tmpsns-ca @0x4a440000): the SM only returns a reading when the TMPSNS `DRrdy` flag is set (`DEV_SM_SensorReadingGet`); our zeroed region never sets it, so the SM returns `SM_ERR_BUSY` and the guest reads `EBUSY` — honest "no data", not a fake temperature. **PMIC** (`pf09`, `pf53_soc`, `pf53_arm`): these are **coarse threshold** sensors, not linear. `PF09_TempGet`/`PF53_TempGet` read the PMIC alarm-status register (`STATUS2_SNS`/`INT_SENSE2`) and map bits→155/140/125/110°C, **none set → 105°C**. Our PMIC register-file returns 0 (no alarm), so the guest reads a steady **105°C = "below the lowest thermal alarm = normal"** — which is exactly what a healthy real board reports; it is faithful, not a frozen constant. (Investigated as a suspected silent-wrong; tracing the SM conversion showed 105°C is the correct "not overheating" bucket. An operator could inject `STATUS2_SNS` alarm bits via a future QOM prop to exercise the thermal governor, but the default is silicon-faithful.) |
