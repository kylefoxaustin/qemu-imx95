...
# small g2d exerciser (g2d_nv12.c) that drives libg2d-dpu through an
# RGBA8888 <-> NV12 conversion, then checks the round trip:
#
#   g2d_nv12 -> libg2d-dpu -> /dev/dri/renderD* (dpu95 SET_CMDLIST)
...
"$CC" --sysroot="$BSP_ROOTFS" -O2 -o "$STAGE/opt/g2d_samples/g2d_nv12" \
    "$HERE/g2d_nv12.c" -L"$BSP_ROOTFS/usr/lib" -l:libg2d.so.2 \
    || { echo "FAIL: could not cross-compile g2d_nv12.c"; exit 1; }
...
EXE = os.path.join(STAGE, "opt/g2d_samples/g2d_nv12")
...
/opt/g2d_samples/g2d_nv12 > /tmp/x.log 2>&1
echo "rc=$?"
grep -aE 'RGB2NV12:|NV122RGB:|G2D-NV12:' /tmp/x.log
echo "=== G2D-NV12-DONE ==="
...
echo "=== g2d NV12-conversion e2e report ==="
sed -n '/G2D-NV12 ===/,/G2D-NV12-DONE/p' "$LOG" | grep -vE '^\[ *[0-9].*\]'

pass=1
grep -aq 'RGB2NV12: PASS' "$LOG" || { echo "  MISS RGBA->NV12 self-check"; pass=0; }
grep -aq 'NV122RGB: PASS' "$LOG" || { echo "  MISS NV12->RGBA round-trip"; pass=0; }

[ "$pass" = 1 ] && {
    echo "PASS: 2D blit RGBA<->NV12 conversion validated through G2D"; exit 0; }
echo "FAIL"; exit 1
