...
#
# i.MX 95 DPU 2D blit engine - scale + rotate end-to-end validation via NXP G2D.
#
# Sibling of run.sh (which validates copy/fill/blend). This one cross-compiles a
# small g2d exerciser (g2d_scale_rot.c) that drives libg2d-dpu through a 2x
# upscale and a 90-degree rotation, then self-checks the destination:
#
#   g2d_scale_rot -> libg2d-dpu -> /dev/dri/renderD* (dpu95 SET_CMDLIST)
#       -> our imx95.dpu blit model (HScaler9/VScaler9 scale, FetchRot9 rotate)
...
"$CC" --sysroot="$BSP_ROOTFS" -O2 -o "$STAGE/opt/g2d_samples/g2d_scale_rot" \
    "$HERE/g2d_scale_rot.c" -L"$BSP_ROOTFS/usr/lib" -l:libg2d.so.2 \
    || { echo "FAIL: could not cross-compile g2d_scale_rot.c"; exit 1; }
...
EXE = os.path.join(STAGE, "opt/g2d_samples/g2d_scale_rot")
...
/opt/g2d_samples/g2d_scale_rot > /tmp/x.log 2>&1
echo "rc=$?"
grep -aE 'SCALE:|ROTATE:|G2D-XFORM:' /tmp/x.log
echo "=== G2D-XFORM-DONE ==="
...
echo "=== g2d scale+rotate e2e report ==="
